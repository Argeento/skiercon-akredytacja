...
import type { GsGuest, TicketInput, WithLabel } from '@/types'
...
const selected = ref<WithLabel<GsGuest> | TicketInput | undefined>(
...
const soldTickets = computed(
  () =>
    tickets.value.filter(
      ticket =>
        ticket.ticketType === 'guest' && ticket.id === selected.value?.id
    ).length
)

watch(multiTicket, multiTicket => {
  resetTicketsToSell()
  addTicketToSell(getDefaultTicket('guest'))
...
const options = computed<Array<WithLabel<GsGuest>>>(() => {
  if (!ticket.value.ticketType) {
    return []
  }

  return people.value.guests
    .filter(
      person =>
        person.tickets > 1 ||
...
<template>
  <div class="card">
    <div class="mb-3">Wyszukaj <i>Gościa</i>:</div>
    <v-select
      ref="vselect"
      :options="options"
      v-model="selected"
    ...
    <div
      v-if="ticket.ticketType === 'guest' && ticket.tickets > 1"
  ...
  <div v-if="ticket.ticketType === 'guest' && ticket.tickets > 1">
    <Counter
